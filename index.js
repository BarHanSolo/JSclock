...
const SECONDS_PROPERTY_NAME = "--seconds";
...
class Clock {
  constructor() {
    ...
    window.addEventListener(
      "mouseup",
      () => (this.windowProperty.isDraggable = false)
    );
    window.addEventListener(
      "mouseout",
      () => (this.windowProperty.isDraggable = false)
  ...
  attachToElements() {
    ...
    this.containersElements = [
      ...document.querySelectorAll(CLOCK_CONTAINER_SELECTOR)
    ];
  ...
  handleResizeWindow() {
    ...
    const scale =
      maxSize > MAX_SIZE_OF_CLOCK
        ? MAX_SIZE_OF_CLOCK / CLOCK_SIZE
        : maxSize / CLOCK_SIZE;
  ...
  clockTick() {
    ...
    this.minutesDisplayElement.textContent =
      minutes <= 9 ? `0${minutes}` : minutes;
    this.secondsDisplayElement.textContent =
      seconds <= 9 ? `0${seconds}` : seconds;
    this.propertyContainer.setProperty(SECONDS_PROPERTY_NAME, seconds);
    this.propertyContainer.setProperty(
      MINUTES_PROPERTY_NAME,
      `${minutes * MINUTES_HAND_SHIFT}deg`
    );
    this.propertyContainer.setProperty(
      HOURS_PROPERTY_NAME,
      `${(hours % 12) * HOURS_HAND_SHIFT}deg`
    );
  }
  handleToogleClock() {
    this.containersElements.forEach((clockElement) =>
      clockElement.classList.toggle(CLOCK_IS_ACTIVE_CLASS)
    );
  }
}
